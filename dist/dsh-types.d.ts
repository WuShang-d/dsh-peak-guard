...
export interface RpcServiceLike {
    handle(channel: string, handler: (endpoint: string, payload: unknown) => Promise<unknown> | unknown, options?: {
        authority?: string;
    }): unknown;
}
...
export interface DshContextLike {
    ...
    on?(event: 'agent/pre-step', listener: (payload: PreStepPayload, next: PreStepNext) => Promise<PreStepDecision>, options?: {
        prepend?: boolean;
    }): unknown;
}
